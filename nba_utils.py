...
def plot_shot_chart(x, y, kind='hex', gridsize=15, norm=None, label='', title=''):
    ...
    if kind == 'hex':
        # Plot hexbin using matplotlib's hexbin function
        plt.hexbin(x=x, y=y, gridsize=gridsize, cmap=cmap, norm=norm)

    else:
        # Customize other plot types here if needed
        pass

    # Set plot size
    plt.gcf().set_size_inches(9, 8)

    # Draw court on the plot
    draw_court(plt.gca())

    # Adjust the axis limits and orientation of the plot to plot half court, with the hoop by the top of the plot
    plt.xlim(-250, 250)
    plt.ylim(422.5, -47.5)
    ...
    # Add Data Source
    plt.text(-250, 442, 'Data source: stats.nba.com via NBAsavant\nPlotting code source: Savvas Tjortjoglou', fontsize=9)

    # Add label
    plt.text(-200, 405, label, fontsize=20)
